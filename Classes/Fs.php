...
class Fs {
	...
	public static function copy(string $origin, string $target): void {
		...
		if ($originIsLocal && is_dir($origin)) static::getFs()->mirror($origin, $target);
		else static::getFs()->copy($origin, $target, TRUE);
	}
	...
	public static function isReadable($files): bool {
		$checker = function (string $filename): bool {
	...
	public static function touch($files, $time = NULL, $atime = NULL): void {
		if (!empty($time) && $time instanceof DateTime) $time = $time->getTimestamp();
		if (!empty($atime) && $atime instanceof DateTime) $atime = $atime->getTimestamp();
	...
	public static function getDirectoryIterator(string $directory, bool $recursive = FALSE, array $options = []): iterable {
		...
		$it = $recursive ?
			new RecursiveIteratorIterator(new RecursiveDirectoryIterator($directory, FilesystemIterator::SKIP_DOTS),
				!empty($options["dirFirst"]) || in_array("dirFirst", $options) ?
	...
	public static function getFs(): Filesystem {
		if (isset(static::$fs)) return static::$fs;
		return static::$fs = new static::$fsClass();
	}
